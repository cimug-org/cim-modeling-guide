...
<tr class="odd">
<td>Rule059</td>
<td>The general rule is that attribute names should be written out completely. This rule describe the exception to the rule in regards to attributes that is defined as standard power system units, e.g. to avoid that an attribute is called "activePower" and declared as type "ActivePower". So, for attributes that correspond to power system unit will just use the relevant unit symbol, in lower camel case (e.g. ratedU), instead of the name of the unit (e.g. ratedVoltage). Any unit multiplier should not be included in the name (e.g. ratedkU is not allowed). The list of the relevant units are: <br><ul><li>Active Power (p)<li>Reactive Power (q)<li>Apparent Power (s)<li>Resistance (r)<li>Reactance (x)<li>Conductance (g)<li>Susceptance (b)<li>Power Factor (pf)<li>Voltage (u)<li>Current (i)<li>Frequency (f)<li>Inductance (l)<li>Capacitance (c)<li>Impedance (z)<li>Admittance (y)<li>Real Energy (e)</ul>For the mentioned in the above list, the symbol is to be appended with "2" for negative-sequence and "0" for zero-sequence properties.<br><br>For the properties that include maximum and minimum, the respective abbreviation "max" (for maximum) and "min" (for minimum) should be used.<br><br>Where properties mentioned in the list are used in a compound term including maximum, rated, nominal, natural, reference, sensitivity and set point, the unit reference should be in the end and ahead of the listed terms.<br><br>The following are recommended naming <br><ul><li>p<li>maxP<li>maxNominalP</ul>The following are not recommended naming <br><ul><li>activePower<li>maximumP<li>ratedMaxP</ul> The attribute name as a whole must also comply with Rule049. <br><br>(NOTE: This rule was introduced as of CIM18. Pre-existing attributes that do not comply with this rule may be left as-is to reduce breaking changes; however, moving forward all new attributes that fall into this category must comply.)</td>
</tr>
<tr class="even">
<td>Rule060</td>
